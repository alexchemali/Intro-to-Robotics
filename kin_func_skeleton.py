...
def skew_3d(omega):
    ...
    if not omega.shape == (3,):
        raise TypeError('omega must be a 3-vector')
    #recall it's always (y,x) for indexing
...
def rotation_3d(omega, theta):
    ...
    omega_len = np.sqrt(np.dot(np.transpose(omega), omega))
    omega_skew = skew_3d(omega)
    i_3 = np.eye(3)
    R = i_3 + omega_skew/(omega_len)*np.sin(omega_len*theta) + np.dot(omega_skew, omega_skew)/(omega_len**2)*(1 - np.cos(omega_len*theta))
...
def hat_3d(xi):
    ...
    if not xi.shape == (6,):
        raise TypeError('omega must be a 6-vector')
    xi_hat=np.zeros((4,4))
    xi_hat[0,1] = -xi[5]
    xi_hat[1,0] =  xi[5]
    xi_hat[0,2] = xi[4]
...
def homog_3d(xi, theta):
    ...
    rot = rotation_3d(xi[3:6],theta)    
    omega = np.array([xi[3:6]])
    omega_hat = skew_3d(xi[3:6])    
    v = np.array([xi[0:3]])
    omega_len = np.linalg.norm(omega,2)

    i_3 = np.eye(3)

    ni = np.dot( (i_3-rot) ,( np.dot(omega_hat,np.transpose(v)) ) )
    no = theta*np.dot(np.dot(np.transpose(omega),omega),np.transpose(v))
    n2 = (ni+no)/(omega_len**2)
...
def prod_exp(xi, theta):
    ...
    """
    # is this just the product of all vectors from pt. (d)?
    return_list = 1
    for i in range(0, len(theta)):
        x = xi.T[i]
        t = theta[i]
        ret = homog_3d(x,t)
        return_list = np.dot(return_list, ret)
